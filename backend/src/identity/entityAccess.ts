...
export const ENTITY_ACCESS: Record<string, EntityClassification> = {
  ...
  CardOnFile: { area: 'BILLING_ACU', sensitivity: 'COMMERCIAL_L3' },
  ...
  // The working set a device carries. Under the same area as the sync it feeds,
  // because a pack is what makes offline capture possible rather than a thing
  // of its own.
  OfflinePack: { area: 'FIELD_EXECUTION' },
  ...
  Group: { area: 'ENTERPRISE_STRUCTURE' },
  GroupRole: { area: 'ENTERPRISE_STRUCTURE' },
  IssuerProfile: { area: 'ENTERPRISE_STRUCTURE' },
  DocumentSequence: { area: 'ENTERPRISE_STRUCTURE' },
  ...
  // A correction the record supports, and what somebody decided about it. It
  // names lost bids and competitors' prices, so it sits at the same
  // classification as the estimates it would correct.
  CalibrationLesson: { area: 'ESTIMATE_TENDER', sensitivity: 'COMMERCIAL_L3' },
