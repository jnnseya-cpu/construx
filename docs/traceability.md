...
| Virus scan | Built as a client, not an engine | `evidence/scanner.ts` speaks clamd's INSTREAM protocol over a socket; the platform holds no signatures of its own and never will. Where none is configured `antivirusScanned` stays false on every record and `antivirusConfigured` false on every read, so zero quarantined is still never readable as nothing infected. Where one is configured and does not answer, the ingestion is refused rather than recorded as unscanned. Structural refusal runs either way: executable magic, EICAR, active markup, an archive carrying a program, a declared compression ratio above 200:1 |
...
| Availability, Scale, Business continuity | **Partial** | Single instance by enforcement, not by hope: a second writer on the same journal volume refuses to start rather than interleaving its appends (`goldenthread/writerlock.ts`). The Postgres schema that would let two instances run is written and verified against a real server — `deploy/postgres/verify.sh` proves append-only, RLS isolation, chain integrity under two concurrent writers, and evidence. The client is not written and `pg` is not going to be added, so the platform still runs on one process. Multi-zone, failover and RPO/RTO remain design only |
