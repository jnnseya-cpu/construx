...
- Limit: the wallet's monthly cap, set from the group by finance. **For AI it reports rather than refuses** — an AI run that passes the cap finishes and the breach is named on the ledger entry, because a reasoning task stopped at a ceiling has spent the money and produced nothing anybody can use. It still refuses non-AI metered work (a document export, a spatial compute), which are fixed jobs priced up front. The hard stop for AI is the **balance**: prepaid only, no ACUs means no AI, and nothing runs a provider on credit. **Thresholds and the limit are told** (§9.3, `usage.threshold` / `usage.limit_reached`): the wallet signals 50 / 80 / 100 per cent once each per month and the limit once per scope, the platform records `ACU_ALERT_RAISED` / `ACU_CAP_BREACHED` on the company's chain, and `billing/usagealerts.ts` notifies the company's administrators and the group's finance and administrators (`acu.threshold`, `acu.limit_reached`) — which is now the whole of what a cap does to an AI run. Queueing of AI jobs at the limit: moot for AI, since nothing is queued at a cap; a document export at the cap is refused rather than queued, and that is not built.
...
| BudgetLimit | `ACUCaps`: monthly, per project, per module, per person. A ceiling that reports AI spend and refuses non-AI metered work — see the limit bullet in §9.3 above |
...
- Holds are reservations against available balance, never negative — for AI as much as for anything else; `settle` charges once and a replayed settlement returns the same entry without moving the balance; `release` after settle is a no-op (commit and release are mutually exclusive). The §10.3 arithmetic is `enterprisegroup.test.ts` AT-20/21 (100 → 70 held, 40 refused, 55 settled → 45 available, 0 held, 55 consumed, replay unchanged).
- Budgets: monthly, per project, per module and — new — per person (`perUserMinor`, set on Billing).
- **reconciliation_required (AT-22):** a remote call that times out after it left (`AI_PROVIDER_TIMEOUT`) parks its hold — neither released nor charged — and the execution is `UNRESOLVED` until the operator reconciles it with evidence. **Unreachable on a default deployment**, and deliberately: `AI_PROVIDER_DEADLINE_MS` is 0, so no deadline is attached to a provider call and nothing times out. It was a hardcoded two minutes, which aborted legitimate long-running perception and reasoning calls and parked them here after the vendor had already done and billed the work. Set a positive deadline and this path is live again (`GET /v1/admin/ai/unreconciled`, `POST /v1/admin/ai/executions/:id/reconcile`: charge the call's cost or release; once; a replayed settlement moves nothing). Worker leases are not needed: AI runs in-process and synchronously.
